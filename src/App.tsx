      ...
      <main className="main-area">
        ...
        {(!inCall || !showCall) && (
          <div className="content-scroll">
            {view === 'Home' && <MinimalHomeView userId={user?.id} recentRooms={hasRoomHistory ? roomList.slice(0, 3) : []} rooms={roomList.filter((room) => room.kind === 'persistent').slice(0, 4)} onCreate={() => setShowCreate(true)} onJoin={joinRoom} onOpenRoom={(room) => { setRoomDetail(room); setView('Rooms') }} onRemoved={(roomId) => { setRoomList((current) => current.filter((item) => item.id !== roomId)); setActiveRoom((current) => current?.id === roomId ? null : current) }} joinError={joinError} />}
...
function MinimalHomeView({ userId, recentRooms, rooms, onCreate, onJoin, onOpenRoom, onRemoved, joinError }: { userId?: string; recentRooms: Room[]; rooms: Room[]; onCreate: () => void; onJoin: (code: string) => void; onOpenRoom: (room: Room) => void; onRemoved: (roomId: string) => void; joinError: string }) {
...
function RecentRoomRemove({ room, userId, onRemoved }: { room: Room; userId?: string; onRemoved: (roomId: string) => void }) {
  ...
  const confirm = async () => {
    ...
    try { if (isOwner) await deleteRoom(room.id); else await leaveRoom(room.id); setOpen(false); onRemoved(room.id) }
    catch { setError(isOwner ? 'Couldn’t delete the room. Try again.' : 'Couldn’t leave the room. Try again.'); setBusy(false) }
  }
  return <><button type="button" className="home-recent-remove" aria-label={isOwner ? `Delete ${room.name}` : `Leave ${room.name}`} title={isOwner ? 'Delete room' : 'Leave room'} onClick={() => { setOpen(true); setError('') }}><Icon name="close" size={12} /></button>{open && <Modal title={isOwner ? 'Delete room' : 'Leave room'} onClose={() => { if (!busy) setOpen(false) }}><p className="room-confirm-title">{isOwner ? `Delete “${room.name}”?` : `Leave “${room.name}”?`}</p><p className="form-hint">{isOwner ? 'This permanently removes the room, its members and all messages.' : 'You will no longer see this room until you join again with its code.'}</p>{error && <p className="form-error" role="alert">{error}</p>}<div className="compact-modal-footer"><button type="button" className="secondary-button" disabled={busy} onClick={() => setOpen(false)}>Cancel</button><button type="button" className="primary-button danger-button" disabled={busy} onClick={() => void confirm()}>{busy ? 'Working…' : isOwner ? 'Delete room' : 'Leave room'}</button></div></Modal>}</>
}
