...
function MinimalPeopleView() {
  const [search, setSearch] = useState('')
  const [addSearch, setAddSearch] = useState('')
  const [showAdd, setShowAdd] = useState(false)
  const normalizedSearch = search.trim().toLowerCase()
  const normalizedAddSearch = addSearch.trim().toLowerCase()
  const filteredPeople = people.filter((person) => !normalizedSearch || person.name.toLowerCase().includes(normalizedSearch) || person.username.toLowerCase().includes(normalizedSearch))
  const addResults = people.filter((person) => !normalizedAddSearch || person.name.toLowerCase().includes(normalizedAddSearch) || person.username.toLowerCase().includes(normalizedAddSearch))
  const openAdd = () => { setAddSearch(''); setShowAdd(true) }
  return <section className="page people-page">
    <div className="people-header"><h1>People</h1><div className="people-header-actions"><label className="people-search"><Icon name="search" size={15} /><span className="sr-only">Search people</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find someone" /></label><button className="primary-button" onClick={openAdd}><Icon name="plus" size={16} />Add person</button></div></div>
    {filteredPeople.length > 0 ? <div className="people-list people-directory" aria-label="People">{filteredPeople.map((person) => <div className="contact-row" key={person.id}><span className={`avatar avatar-${person.tone}`}>{person.initials}</span><span className="person-info"><strong>{person.name}</strong><small>@{person.username}</small></span><span className={`presence-dot ${person.active ? 'active' : ''}`} title={person.active ? 'Online' : 'Offline'} aria-label={person.active ? 'Online' : 'Offline'} /><button className="more-button contact-more" aria-label={`More actions for ${person.name}`}><Icon name="more" size={16} /></button></div>)}</div> : <div className="people-empty"><p>No people found.</p><button className="secondary-button" onClick={openAdd}>Add someone</button></div>}
    {showAdd && <Modal title="Add someone" onClose={() => setShowAdd(false)}><div className="add-person-modal"><label className="form-label" htmlFor="add-person-search">Search by username</label><div className="add-person-search"><Icon name="search" size={15} /><input id="add-person-search" value={addSearch} onChange={(event) => setAddSearch(event.target.value)} placeholder="@username" autoFocus /></div>{addResults.slice(0, 1).map((person) => <div className="add-person-result" key={person.id}><span className={`avatar avatar-${person.tone}`}>{person.initials}</span><span><strong>{person.name}</strong><small>@{person.username}</small></span><button className="secondary-button" onClick={() => setShowAdd(false)}>Add</button></div>)}{addResults.length === 0 && <p className="form-hint">No person found.</p>}</div></Modal>}
  </section>
}
