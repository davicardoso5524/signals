...
function SignalWorkspace() {
  const { profile, profileLoading, user, refreshProfile, signOut } = useAuth()
  ...
  useEffect(() => {
    ...
    const refresh = () => { void listRooms().then((records) => {
      const remoteRooms = records.map(mapRoom)
      setRoomList(remoteRooms)
      setHasRoomHistory(remoteRooms.length > 0)
    }).catch(() => undefined) }
    refresh()
    const unsubscribe = subscribeToRooms(refresh)
    return unsubscribe
  }, [user])
  ...
  return (
    <div className="app-shell">
      ...
      {showFriendRequests && <FriendRequestsModal requests={friendships.filter((item) => item.direction === 'incoming' && item.status === 'pending')} onClose={() => setShowFriendRequests(false)} onChanged={refreshFriendships} />}
...
function MinimalPeopleView({ userId, conversations, friendships, onOpen, onChanged, onFriendsChanged }: { userId?: string; conversations: Conversation[]; friendships: Friendship[]; onOpen: (conversation: Conversation) => void; onChanged: () => void; onFriendsChanged: () => void }) {
  ...
  const addFriend = async (person: Profile) => { setLoading(true); setError(''); try { await sendFriendRequest(person.id); onFriendsChanged() } catch { setError('Couldn\'t send the friend request. Try again.') } finally { setLoading(false) } }
  ...
  const friendshipFor = (personId: string) => friendships.find((item) => item.person.id === personId)
  return <section className="page people-page"><div className="people-header"><div><p className="eyebrow">Workspace</p><h1>Friends<span className="accent-dot">.</span></h1></div><div className="people-header-actions"><label className="people-search"><Icon name="search" size={15} /><span className="sr-only">Find conversations and friends</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find someone…" /></label><button className="primary-button" onClick={openNew}><Icon name="plus" size={16} />New</button></div></div>
    {acceptedFriends.length > 0 && <><div className="friends-section-label">Friends <span>{acceptedFriends.length}</span></div><div className="friends-grid">{acceptedFriends.filter((item) => `${item.person.display_name} ${item.person.username}`.toLowerCase().includes(search.trim().toLowerCase())).map((item) => <div className="friend-card" key={item.id}><span className="avatar avatar-jade">{item.person.avatar_url ? <img src={item.person.avatar_url} alt={`${item.person.display_name}'s avatar`} /> : initialsFor(item.person.display_name)}</span><span className="conversation-info"><strong>{item.person.display_name}</strong><small>@{item.person.username}</small></span><button className="secondary-button small-button" onClick={() => { const conversation = conversations.find((entry) => entry.type === 'direct' && entry.person?.id === item.person.id); if (conversation) onOpen(conversation); else void chooseDirect(item.person) }}>Message</button></div>)}</div></>}
    <div className="friends-section-label">Conversations <span>{visible.length}</span></div>{visible.length ? <div className="people-list people-directory" aria-label="Conversations">{visible.map((conversation) => { const title = conversation.type === 'direct' ? conversation.person?.display_name || 'Conversation' : conversation.name || 'Group'; const subtitle = conversation.type === 'direct' ? `@${conversation.person?.username || ''}` : `${conversation.memberCount} people`; const latest = conversation.latestMessage; const preview = latest ? `${latest.sender_id === userId ? 'You: ' : ''}${latest.content}` : 'No messages yet.'; return <button className="conversation-row" key={conversation.id} onClick={() => onOpen(conversation)}><span className="avatar avatar-jade">{conversation.type === 'direct' && conversation.person?.avatar_url ? <img src={conversation.person.avatar_url} alt={`${title}'s avatar`} /> : initialsFor(title)}</span><span className="conversation-info"><strong>{title}</strong><small>{subtitle}</small><span>{preview}</span></span>{latest && <time>{formatConversationTime(latest.created_at)}</time>}</button>})}</div> : <div className="people-empty"><p>No conversations yet.</p><small>Find someone or create a group to start talking.</small></div>}
...
function FriendsView({ userId, conversations, friendships, onOpen, onChanged, onFriendsChanged }: { userId?: string; conversations: Conversation[]; friendships: Friendship[]; onOpen: (conversation: Conversation) => void; onChanged: () => void; onFriendsChanged: () => void }) {
  const [search, setSearch] = useState(''); const [query, setQuery] = useState(''); const [results, setResults] = useState<Profile[]>([]); const [modalOpen, setModalOpen] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState('')
  const accepted = friendships.filter((item) => item.status === 'accepted')
  const find = async () => { if (!userId || !query.trim()) return; setLoading(true); setError(''); try { setResults(await findProfiles(query, userId)) } catch { setError('Couldn\'t search people. Try again.') } finally { setLoading(false) } }
  const message = async (person: Profile) => { if (!userId) return; setLoading(true); try { const existing = conversations.find((item) => item.type === 'direct' && item.person?.id === person.id); if (existing) onOpen(existing); else { const conversation = await getOrCreateDirect(person.id); onChanged(); onOpen({ ...conversation, type: 'direct', person, memberCount: 2, latestMessage: null }) } } catch { setError('Couldn\'t open the conversation. Try again.') } finally { setLoading(false) } }
  const add = async (person: Profile) => { setLoading(true); setError(''); try { await sendFriendRequest(person.id); onFriendsChanged() } catch { setError('Couldn\'t send the friend request. Try again.') } finally { setLoading(false) } }
  const friendshipFor = (personId: string) => friendships.find((item) => item.person.id === personId)
  const visible = conversations.filter((item) => `${item.name || ''} ${item.person?.display_name || ''} ${item.person?.username || ''}`.toLowerCase().includes(search.trim().toLowerCase()))
  return <section className="page people-page"><div className="people-header"><div><p className="eyebrow">Workspace</p><h1>Friends<span className="accent-dot">.</span></h1><p className="intro-copy">Your friends and direct conversations.</p></div><div className="people-header-actions"><label className="people-search"><Icon name="search" size={15} /><span className="sr-only">Find conversations and friends</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find someone…" /></label><button className="primary-button" onClick={() => { setModalOpen(true); setError('') }}><Icon name="plus" size={16} />Add friend</button></div></div>
    {accepted.length > 0 && <><div className="friends-section-label">Friends <span>{accepted.length}</span></div><div className="friends-grid">{accepted.filter((item) => `${item.person.display_name} ${item.person.username}`.toLowerCase().includes(search.trim().toLowerCase())).map((item) => <div className="friend-card" key={item.id}><span className="avatar avatar-jade">{item.person.avatar_url ? <img src={item.person.avatar_url} alt={`${item.person.display_name}'s avatar`} /> : initialsFor(item.person.display_name)}</span><span className="conversation-info"><strong>{item.person.display_name}</strong><small>@{item.person.username}</small></span><button className="secondary-button small-button" onClick={() => void message(item.person)}>Message</button></div>)}</div></>}
    <div className="friends-section-label">Conversations <span>{visible.length}</span></div>{visible.length ? <div className="people-list people-directory" aria-label="Direct conversations">{visible.map((conversation) => { const title = conversation.type === 'direct' ? conversation.person?.display_name || 'Conversation' : conversation.name || 'Group'; const latest = conversation.latestMessage; return <button className="conversation-row" key={conversation.id} onClick={() => onOpen(conversation)}><span className="avatar avatar-jade">{conversation.type === 'direct' && conversation.person?.avatar_url ? <img src={conversation.person.avatar_url} alt={`${title}'s avatar`} /> : initialsFor(title)}</span><span className="conversation-info"><strong>{title}</strong><small>{conversation.type === 'direct' ? `@${conversation.person?.username || ''}` : `${conversation.memberCount} people`}</small><span>{latest ? `${latest.sender_id === userId ? 'You: ' : ''}${latest.content}` : 'No messages yet.'}</span></span>{latest && <time>{formatConversationTime(latest.created_at)}</time>}</button>})}</div> : <div className="people-empty"><p>No conversations yet.</p><small>Find someone by username to start talking.</small></div>}
    {modalOpen && <Modal title="Find someone" onClose={() => setModalOpen(false)}><label className="form-label" htmlFor="friend-username">Username</label><div className="add-person-search"><Icon name="search" size={15} /><input id="friend-username" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void find() } }} placeholder="@username" autoFocus /><button className="text-button" type="button" onClick={() => void find()}>Find</button></div>{loading && <p className="form-hint">Searching…</p>}{error && <p className="form-error" role="alert">{error}</p>}{results.map((person) => { const friendship = friendshipFor(person.id); const action = friendship?.status === 'accepted' ? 'Friends' : friendship?.status === 'pending' ? friendship.direction === 'incoming' ? 'Respond in notifications' : 'Pending' : 'Add friend'; return <div className="add-person-result" key={person.id}><span className="avatar avatar-jade">{person.avatar_url ? <img src={person.avatar_url} alt={`${person.display_name}'s avatar`} /> : initialsFor(person.display_name)}</span><span><strong>{person.display_name}</strong><small>@{person.username}</small></span><button className="secondary-button" disabled={Boolean(friendship && friendship.status !== 'declined') || loading} onClick={() => void add(person)}>{action}</button><button className="secondary-button" disabled={loading} onClick={() => void message(person)}>Message</button></div>})}</Modal>}
  </section>
}

function FriendRequestsModal({ requests, onClose, onChanged }: { requests: Friendship[]; onClose: () => void; onChanged: () => void }) {
  const [busy, setBusy] = useState<string | null>(null); const [error, setError] = useState('')
  const respond = async (request: Friendship, accept: boolean) => { setBusy(request.id); setError(''); try { await respondToFriendRequest(request.id, accept); onChanged() } catch { setError('Couldn\'t update this request. Try again.') } finally { setBusy(null) } }
  return <Modal title="Friend requests" onClose={onClose}>{requests.length ? <div className="friend-requests-list">{requests.map((request) => <div className="friend-request-row" key={request.id}><span className="avatar avatar-jade">{request.person.avatar_url ? <img src={request.person.avatar_url} alt={`${request.person.display_name}'s avatar`} /> : initialsFor(request.person.display_name)}</span><span className="conversation-info"><strong>{request.person.display_name}</strong><small>@{request.person.username} wants to be your friend</small></span><button className="secondary-button" disabled={busy !== null} onClick={() => void respond(request, false)}>Decline</button><button className="primary-button" disabled={busy !== null} onClick={() => void respond(request, true)}>Accept</button></div>)}</div> : <div className="people-empty"><p>No pending requests.</p><small>New requests will appear here.</small></div>}{error && <p className="form-error" role="alert">{error}</p>}</Modal>
}

function FriendsViewWithActions({ userId, conversations, friendships, onOpen, onChanged, onFriendsChanged }: { userId?: string; conversations: Conversation[]; friendships: Friendship[]; onOpen: (conversation: Conversation) => void; onChanged: () => void; onFriendsChanged: () => void }) {
  const [search, setSearch] = useState(''); const [query, setQuery] = useState(''); const [results, setResults] = useState<Profile[]>([]); const [modalOpen, setModalOpen] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState('')
  const accepted = friendships.filter((item) => item.status === 'accepted')
  const friendshipFor = (personId: string) => friendships.find((item) => item.person.id === personId)
  const find = async () => { if (!userId || !query.trim()) return; setLoading(true); setError(''); try { setResults(await findProfiles(query, userId)) } catch { setError('Couldn\'t search people. Try again.') } finally { setLoading(false) } }
  ...
  const add = async (person: Profile) => { setLoading(true); setError(''); try { await sendFriendRequest(person.id); onFriendsChanged() } catch { setError('Couldn\'t send the friend request. Try again.') } finally { setLoading(false) } }
  const visible = conversations.filter((item) => `${item.name || ''} ${item.person?.display_name || ''} ${item.person?.username || ''}`.toLowerCase().includes(search.trim().toLowerCase()))
  const removeConversation = async (conversation: Conversation) => { try { await deleteConversationForMe(conversation.id); onChanged() } catch { setError('Couldn\'t delete the conversation. Try again.') } }
  const removeFriendship = async (friendship: Friendship) => { try { await removeFriend(friendship.id); onFriendsChanged() } catch { setError('Couldn\'t remove this friend. Try again.') } }
  return <section className="page people-page"><div className="people-header"><div><p className="eyebrow">Workspace</p><h1>Friends<span className="accent-dot">.</span></h1><p className="intro-copy">Your friends and direct conversations.</p></div><div className="people-header-actions"><label className="people-search"><Icon name="search" size={15} /><span className="sr-only">Find conversations and friends</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find someone…" /></label><button className="primary-button" onClick={() => { setModalOpen(true); setError('') }}><Icon name="plus" size={16} />Add friend</button></div></div>
    {accepted.length > 0 && <><div className="friends-section-label">Friends <span>{accepted.length}</span></div><div className="friends-grid">{accepted.filter((item) => `${item.person.display_name} ${item.person.username}`.toLowerCase().includes(search.trim().toLowerCase())).map((item) => <div className="friend-card" key={item.id}><span className="avatar avatar-jade">{item.person.avatar_url ? <img src={item.person.avatar_url} alt={`${item.person.display_name}'s avatar`} /> : initialsFor(item.person.display_name)}</span><span className="conversation-info"><strong>{item.person.display_name}</strong><small>@{item.person.username}</small></span><button className="secondary-button small-button" onClick={() => void message(item.person)}>Message</button><FriendMenu onRemove={() => void removeFriendship(item)} /></div>)}</div></>}
    <div className="friends-section-label">Conversations <span>{visible.length}</span></div>{visible.length ? <div className="people-list people-directory" aria-label="Direct conversations">{visible.map((conversation) => { const title = conversation.type === 'direct' ? conversation.person?.display_name || 'Conversation' : conversation.name || 'Group'; const latest = conversation.latestMessage; return <div className="conversation-row" key={conversation.id}><button className="conversation-open" onClick={() => onOpen(conversation)}><span className="avatar avatar-jade">{conversation.type === 'direct' && conversation.person?.avatar_url ? <img src={conversation.person.avatar_url} alt={`${title}'s avatar`} /> : initialsFor(title)}</span><span className="conversation-info"><strong>{title}</strong><small>{conversation.type === 'direct' ? `@${conversation.person?.username || ''}` : `${conversation.memberCount} people`}</small><span>{latest ? `${latest.sender_id === userId ? 'You: ' : ''}${latest.content}` : 'No messages yet.'}</span></span>{latest && <time>{formatConversationTime(latest.created_at)}</time>}</button><ConversationMenu onDelete={() => void removeConversation(conversation)} /></div>})}</div> : <div className="people-empty"><p>No conversations yet.</p><small>Find someone by username to start talking.</small></div>}
    {error && <p className="form-error" role="alert">{error}</p>}
    {modalOpen && <Modal title="Find someone" onClose={() => setModalOpen(false)}><label className="form-label" htmlFor="friend-username-actions">Username</label><div className="add-person-search"><Icon name="search" size={15} /><input id="friend-username-actions" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void find() } }} placeholder="@username" autoFocus /><button className="text-button" type="button" onClick={() => void find()}>Find</button></div>{loading && <p className="form-hint">Searching…</p>}{results.map((person) => { const friendship = friendshipFor(person.id); const action = friendship?.status === 'accepted' ? 'Friends' : friendship?.status === 'pending' ? friendship.direction === 'incoming' ? 'Respond in notifications' : 'Pending' : 'Add friend'; return <div className="add-person-result" key={person.id}><span className="avatar avatar-jade">{person.avatar_url ? <img src={person.avatar_url} alt={`${person.display_name}'s avatar`} /> : initialsFor(person.display_name)}</span><span><strong>{person.display_name}</strong><small>@{person.username}</small></span><button className="secondary-button" disabled={Boolean(friendship && friendship.status !== 'declined') || loading} onClick={() => void add(person)}>{action}</button><button className="secondary-button" disabled={loading} onClick={() => void message(person)}>Message</button></div>})}</Modal>}
  </section>
}

function FriendMenu({ onRemove }: { onRemove: () => void }) { const [open, setOpen] = useState(false); const [confirm, setConfirm] = useState(false); return <div className="friend-menu"><button className="more-button" type="button" aria-label="Friend actions" aria-expanded={open} onClick={() => setOpen((value) => !value)}><Icon name="more" size={16} /></button>{open && <div className="friend-menu-popover" role="menu"><button type="button" role="menuitem" onClick={() => { setOpen(false); setConfirm(true) }}>Remove friend</button></div>}{confirm && <Modal title="Remove friend" onClose={() => setConfirm(false)}><p className="form-hint">This removes the friendship, but keeps your direct conversation.</p><div className="compact-modal-footer"><button type="button" className="secondary-button" onClick={() => setConfirm(false)}>Cancel</button><button type="button" className="primary-button danger-button" onClick={() => { setConfirm(false); onRemove() }}>Remove friend</button></div></Modal>}</div> }
function ConversationMenu({ onDelete }: { onDelete: () => void }) { const [open, setOpen] = useState(false); const [confirm, setConfirm] = useState(false); return <div className="friend-menu"><button className="more-button" type="button" aria-label="Conversation actions" aria-expanded={open} onClick={() => setOpen((value) => !value)}><Icon name="more" size={16} /></button>{open && <div className="friend-menu-popover" role="menu"><button type="button" role="menuitem" onClick={() => { setOpen(false); setConfirm(true) }}>Delete conversation</button></div>}{confirm && <Modal title="Delete conversation" onClose={() => setConfirm(false)}><p className="form-hint">This hides the conversation for you. If the other person also deletes it, the conversation and messages are permanently removed.</p><div className="compact-modal-footer"><button type="button" className="secondary-button" onClick={() => setConfirm(false)}>Cancel</button><button type="button" className="primary-button danger-button" onClick={() => { setConfirm(false); onDelete() }}>Delete conversation</button></div></Modal>}</div> }
...
function RoomDetailsView({ room, userId, authorName, username, avatarUrl, callActive, onBack, onStartCall, onOpenCall, onCopy, copied, onRenamed, onExited }: { room: Room; userId?: string; authorName: string; username: string; avatarUrl: string; callActive: boolean; onBack: () => void; onStartCall: () => void; onOpenCall: () => void; onCopy: () => void; copied: boolean; onRenamed: (room: Room) => void; onExited: () => void }) {
  ...
  const currentMember: RoomMemberRecord | null = userId ? { user_id: userId, username, display_name: authorName, avatar_url: avatarUrl || null, joined_at: '' } : null
...
function ProfileSettings({ user, profile, onProfileUpdated }: { user: User | null; profile: SupabaseProfile | null; onProfileUpdated: () => Promise<void> }) {
  ...
  useEffect(() => { setDisplayName(profile?.display_name || user?.user_metadata?.display_name || ''); setAvatarUrl(profile?.avatar_url || '') }, [profile?.id, profile?.display_name, profile?.avatar_url, user?.id])

  const convertToWebp = (file: File) => new Promise<Blob>((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('Image conversion failed')), 'image/webp', 0.86)
    }
  ...
  const saveProfile = async () => {
    if (!supabase || !user || !displayName.trim()) { setError('Enter a display name.'); return }
    setBusy(true); setMessage(''); setError('')
    try {
      const { error: updateError } = await supabase.from('profiles').update({ display_name: displayName.trim(), avatar_url: avatarUrl || null }).eq('id', user.id)
      if (updateError) throw updateError
      await onProfileUpdated()
      setMessage('Profile updated.')
    } catch (updateError) { setError(updateError instanceof Error ? updateError.message : 'Couldn’t update the profile.') }
    finally { setBusy(false) }
  }

  const uploadAvatar = async (file: File) => {
    if (!supabase || !user) return
    if (!/^image\/(jpeg|png|webp)$/.test(file.type)) { setError('Choose a JPG, JPEG, PNG or WebP image.'); return }
    setBusy(true); setMessage(''); setError('')
    try {
      const webp = await convertToWebp(file)
      const path = `${user.id}/${crypto.randomUUID()}.webp`
      const { error: uploadError } = await supabase.storage.from('avatars').upload(path, webp, { contentType: 'image/webp', cacheControl: '3600', upsert: false })
      ...
      setAvatarUrl(data.publicUrl); await onProfileUpdated(); setMessage('')
    } catch (uploadError) { setError(uploadError instanceof Error ? uploadError.message : 'Couldn’t upload the avatar.') }
    finally { setBusy(false) }
  }

  const changePassword = async () => {
    if (!supabase || password.length < 6) { setError('Use a password with at least 6 characters.'); return }
    setBusy(true); setMessage(''); setError('')
    try { const { error: passwordError } = await supabase.auth.updateUser({ password }); if (passwordError) throw passwordError; setPassword(''); setMessage('Password updated.') }
    catch (passwordError) { setError(passwordError instanceof Error ? passwordError.message : 'Couldn’t update the password.') }
    finally { setBusy(false) }
  }

  const nameForAvatar = displayName || profile?.username || user?.user_metadata?.username || user?.email?.split('@')[0] || '?'
  const usernameForDisplay = profile?.username?.trim() || user?.user_metadata?.username?.trim() || ''
  return <><SettingsSection title="Public profile" label="Profile"><div className="profile-settings-head"><span className="avatar avatar-jade profile-settings-avatar">{avatarUrl ? <img src={avatarUrl} alt="Current profile avatar" /> : initialsFor(nameForAvatar)}</span><div><strong>{nameForAvatar}</strong><small>{usernameForDisplay ? `@${usernameForDisplay}` : 'Username unavailable'} · {user?.email || 'Email unavailable'}</small></div><button type="button" className="secondary-button" disabled={busy} onClick={() => fileRef.current?.click()}>Add image</button><input ref={fileRef} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadAvatar(file); event.currentTarget.value = '' }} /></div><label className="form-label" htmlFor="profile-display-name">Display name</label><input id="profile-display-name" className="text-input" value={displayName} onChange={(event) => setDisplayName(event.target.value)} maxLength={80} /><label className="form-label" htmlFor="profile-username">Username</label><input id="profile-username" className="text-input" value={usernameForDisplay} readOnly aria-readonly="true" /><label className="form-label" htmlFor="profile-email">Email</label><input id="profile-email" className="text-input" type="email" value={user?.email || ''} readOnly aria-readonly="true" /><div className="compact-modal-footer"><span className="form-hint">Images are converted to WebP before upload.</span><button type="button" className="primary-button" disabled={busy || !displayName.trim()} onClick={() => void saveProfile()}>Save profile</button></div></SettingsSection><SettingsSection title="Password" label="Security"><label className="form-label" htmlFor="profile-password">New password</label><input id="profile-password" className="text-input" type="password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={6} autoComplete="new-password" /><div className="compact-modal-footer"><span className="form-hint">Use at least 6 characters.</span><button type="button" className="primary-button" disabled={busy || password.length < 6} onClick={() => void changePassword()}>Change password</button></div></SettingsSection>{(message || error) && <p className={error ? 'form-error' : 'auth-success'} role={error ? 'alert' : 'status'}>{error || message}</p>}</>
}
