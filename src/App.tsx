...
function RoomListActions({ room, userId, onRenamed, onExited }: { room: Room; userId?: string; onRenamed: (room: Room) => void; onExited: (roomId: string) => void }) {
  ...
  const leaveOrDelete = async () => {
    if (!window.confirm(isOwner ? `Delete ${room.name}? This also removes its messages.` : `Leave ${room.name}?`)) return
    setBusy(true); setError('')
    try { if (isOwner) await deleteRoom(room.id); else await leaveRoom(room.id); onExited(room.id) }
    catch { setError(isOwner ? 'Couldn’t delete the room. Try again.' : 'Couldn’t leave the room. Try again.'); setBusy(false) }
  }
  return <div className="room-actions-wrap"><button className="more-button room-more" aria-label={`More actions for ${room.name}`} aria-expanded={open} onClick={() => { setOpen((current) => !current); setError('') }}><Icon name="more" size={16} /></button>{open && <div className="room-actions-menu" role="menu">{isOwner && <button type="button" onClick={() => { setRenameValue(room.name); setRenameOpen(true); setOpen(false) }}>Edit room name</button>}<button type="button" className="is-danger" disabled={busy} onClick={() => void leaveOrDelete()}>{isOwner ? 'Delete room' : 'Leave room'}</button>{error && <span className="room-action-error" role="alert">{error}</span>}</div>}{renameOpen && <Modal title="Edit room name" onClose={() => { if (!busy) setRenameOpen(false) }}><label className="form-label" htmlFor={`rename-room-${room.id}`}>Room name</label><input id={`rename-room-${room.id}`} className="text-input" value={renameValue} onChange={(event) => setRenameValue(event.target.value)} maxLength={80} autoFocus />{error && <p className="form-error" role="alert">{error}</p>}<div className="compact-modal-footer"><button type="button" className="secondary-button" disabled={busy} onClick={() => setRenameOpen(false)}>Cancel</button><button type="button" className="primary-button" disabled={busy || !renameValue.trim()} onClick={() => void saveName()}>Save</button></div></Modal>}</div>
}
